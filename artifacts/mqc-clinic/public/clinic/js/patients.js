...
function optionListHtml(options, selected=[], group="option"){
  return options.map(o=>`<label style="display:inline-flex;align-items:center;gap:6px;margin:0 14px 8px 0;font-size:12.5px"><input type="checkbox" data-check-group="${group}" data-check="${escapeHtml(o)}" ${selected.includes(o)?"checked":""}>${escapeHtml(o)}</label>`).join("");
}
...
function openConsultationForm(student, existing=null){
  ...
  <div class="modal-body"><form id="consult-form" class="form-grid">
    ...
    <div class="f-field full"><label>Symptoms</label><div style="padding-top:5px">${optionListHtml(SYMPTOMS,f.symptoms||[],"symptom")}<label style="display:inline-flex;align-items:center;gap:6px;font-size:12.5px"><input type="checkbox" id="cf-symptom-other" ${f.symptoms?.includes("Other")?"checked":""}>Other</label></div></div>
    ...
    <div class="f-field full"><label>Nurse's Assessment</label><div style="padding-top:5px">${optionListHtml(["Stable","Weak","Alert","Drowsy","In Pain"],f.assessment?[f.assessment]:[],"assessment")}<label style="display:inline-flex;align-items:center;gap:6px;font-size:12.5px"><input type="checkbox" id="cf-assess-other" ${f.assessmentOther?"checked":""}>Other</label></div></div>
    <div class="f-field full"><label>Nurse's Notes / Assessment</label><textarea id="cf-notes">${escapeHtml(f.notes||"")}</textarea></div>
    <div class="f-field full"><label>Treatment / Intervention</label><div style="padding-top:5px">${optionListHtml(INTERVENTIONS,f.interventions||[],"intervention") }<label style="display:inline-flex;align-items:center;gap:6px;font-size:12.5px"><input type="checkbox" id="cf-intervention-other" ${f.interventions?.includes("Other")?"checked":""}>Other</label></div></div>
    ...
    <div class="f-field full"><label>Parent / Guardian Notification</label><div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
      ...
      <label>Method<select id="cf-guardian-method">${["phone call","sms/text","messenger","in person"].map(x=>`<option ${f.guardianMethod===x?"selected":""}>${x}</option>`).join("")}</select></label>
