...
function bindSettings(){
  ...
  document.getElementById("reset-data-btn")?.addEventListener("click",()=>confirmDialog({title:"Clear patient and visit data?",msg:"This removes all patient records, visit history, and saved audit entries from Supabase. Inventory and user accounts remain available.",okLabel:"Clear Records",onConfirm:async()=>{STUDENTS.splice(0);CONSULTATIONS=[];AUDIT_LOGS=[];await saveToClinicState(true);toast("Records cleared","Patient, visit, and audit records have been removed.","warn");render();}}));
}