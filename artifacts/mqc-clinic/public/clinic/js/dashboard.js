...
function renderDashboard(){
  const lowStock=MEDICINES.filter(m=>!m.deleted&&(m.qty===0||m.qty<=(state.settings.lowStockThreshold||15)));
  ...
  <div class="two-col" id="alerts-section" style="margin-top:22px"><div class="card"><div class="panel-title"><h4><span style="color:var(--amber);display:inline-flex">${ICONS.alert}</span> Low Stock Alert</h4></div>${lowStockAlertHtml(lowStock)}</div>
  <div class="card"><div class="panel-title"><h4><span style="color:var(--coral);display:inline-flex">${ICONS.alert}</span> Expiring Medicine Alert</h4></div>${expiringAlertHtml(expiring)}</div></div>
...
function lowStockAlertHtml(list){
  ...
  return `<div class="table-wrap"><table><thead><tr><th>Code</th><th>Name</th><th>Quantity</th><th>Supplier</th></tr></thead><tbody>${list.map(m=>`<tr data-open-med5="${m.code}"><td class="mono">${m.code}</td><td>${escapeHtml(m.name)}</td><td class="mono">${m.qty} ${m.unit}${m.qty===0?" — Out of stock":""}</td><td>${escapeHtml(m.supplier)}</td></tr>`).join("")}</tbody></table></div>`;
}
