...
async function upsertTableRows(tableName: string, rows: Array<Record<string, unknown>>) {
  if (!rows.length) return;
  const response = await supabaseRequest(`${tableName}?on_conflict=id`, {
    method: "POST",
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    if (Array.isArray(snapshot.students)) {
      await upsertTableRows("patients", snapshot.students.map((row) => normalizePatientRow(row as Record<string, unknown>)));
    }
    if (Array.isArray(snapshot.medicines)) {
      await upsertTableRows("medicines", snapshot.medicines.map((row) => normalizeMedicineRow(row as Record<string, unknown>)));
    }
    if (Array.isArray(snapshot.equipment)) {
      await upsertTableRows("equipment", snapshot.equipment.map((row) => normalizeEquipmentRow(row as Record<string, unknown>)));
    }
    if (Array.isArray(snapshot.consultations)) {
      await upsertTableRows("clinical_visits", snapshot.consultations.map((row) => normalizeVisitRow(row as Record<string, unknown>)));
    }
    if (Array.isArray(snapshot.users)) {
      await upsertTableRows("clinic_users", snapshot.users.map((row) => normalizeUserRow(row as Record<string, unknown>)));
    }
    if (Array.isArray(snapshot.auditLogs)) {
      await upsertTableRows("audit_logs", snapshot.auditLogs.map((row) => normalizeAuditLogRow(row as Record<string, unknown>)));
    }
    if (snapshot.settings && typeof snapshot.settings === "object") {
      const settings = snapshot.settings as Record<string, unknown>;
      await upsertTableRows("clinic_settings", [{
        id: 1,
